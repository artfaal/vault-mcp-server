...
| `fix(kv): accept delete_secret calls that omit the optional key` | `delete_secret` без параметра `key` отвечал `Missing or invalid 'key' parameter` вместо того, чтобы удалить секрет целиком, — хендлер приводил к строке отсутствующее значение, а дефолт из схемы в аргументы не попадает. Теперь отсутствующий (или `null`) ключ и означает «удалить весь секрет», а значение не того типа по-прежнему отвергается: тот, кто целился в один ключ, не снесёт молча весь секрет | ветка есть, PR не открыт |

Ветки `pr/read-secret-metadata`, `pr/enable-vault-operations`, `pr/write-secret-metadata` и `pr/delete-secret-optional-key` — те же патчи, по одному на ветку; из первых двух открыты upstream-PR. Примет upstream все четыре — форк можно сворачивать, вернувшись на официальный образ.
